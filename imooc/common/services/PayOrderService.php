...
class PayOrderService extends  BaseService {
	...
	public static function createPayOrder( $member_id,$items = [],$params = []){
		$total_price = 0;
		$continue_cnt = 0;//退出的数量
		//防止前端数据写得不好，用服务来验证。出现小于0是由于控制器验证不严格导致
		...
		$discount = isset( $params['discount'] )?$params['discount']:0;//优惠金额
		...
		try{
			...
			//乐观锁： update book set stock =2 where id =1 ande stock =5;
