...
use app\models\members\MemberCart;
...
class ProductController extends BaseController
{
    ...
    public function actionOrder()
    {
        ...
        //防止数据被伪造
        $book_mapping = Book::find()->where([ 'id' => $book_ids ,'status' => 1])->indexBy("id")->all();
        if( !$book_mapping ){
            return $this->renderJSON([],"请选择商品之后在提交~~",-1);
        }
        ...
        if( !$ret ){
            return $this->renderJSON([],"提交失败11，失败原因：".PayOrderService::getLastErrorMsg(),-1 );
        }
   ...
   //加入购物车
    public function actionCart(){
        ...
        $cart_info = MemberCart::find()
            ->where([ 'member_id' => $this->current_user['id'],'book_id' => $book_id ])
            ->one();
        if( $cart_info  ){
    ...
    public function actionSearch(){
        ...
        return $this->renderJson( [ 'da' => $data ,'has_next' => ( count( $data ) == 4 )?1:0 ] );
    }
